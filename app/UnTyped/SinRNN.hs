...
main = do
  ...
  initModel <- sample $ SinRNNSpec (Device CPU 0) 2
  -- | setting
  let numEpochs = 100
  ...
  ((trainedModel, _), losses) <- mapAccumM [1..numEpochs] (initModel, optim) $ \epoc (model, opt) -> do
    let x_tensor = fmap asTensor sin_t
        t_tensor = fmap asTensor sin_t1
        prediction = sinRNN model x_tensor
        loss = mseLoss (stack (Dim 0) prediction) (stack (Dim 0) t_tensor)
  ...
  let x_tensor = fmap asTensor cos_t
      pred = fmap toDouble (sinRNN trainedModel x_tensor)
  plotWave "untyped-inRNN_eval" (times, cos_t1) (times, pred)
  return ()
